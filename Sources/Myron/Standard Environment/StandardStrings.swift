...
extension StandardStrings {
    ...
    static func explode(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        let str = try args.unwrap1(location).unwrapString(location)
        let pieces = str.map { ch in Value.string(String(ch)) }
        return .list(pieces)
    }

    static func implode(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        try args.mustHaveAtLeast(1, location)
        let sep: String
        let elements: [Value]

        if args[0].kind == .string {
            guard args.count == 2 else { throw MyronError(.unexpectedArity, at: location) }
            sep = try args[0].unwrapString(location)
            elements = try args[1].unwrapList(location)
        } else {
            guard args.count == 1 else { throw MyronError(.unexpectedArity, at: location) }
            sep = ""
    ...
    static func string(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        let src = try args.unwrap1(location)
        if src.kind == .string { return src }
        return .string(src.description)
    }

    static func lowercase(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        let str = try args.unwrap1(location).unwrapString(location)
        return .string(str.lowercased())
    }

    static func uppercase(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        let str = try args.unwrap1(location).unwrapString(location)
        return .string(str.uppercased())
    }

    static func trim(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        let str = try args.unwrap1(location).unwrapString(location)
        return .string(str.trimmingCharacters(in: .whitespacesAndNewlines))
    }

    static func lines(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        let str = try args.unwrap1(location).unwrapString(location)
        let lines = str
    ...
    static func words(args: [Value], apply: Applier, location: Range<Int>?) throws -> Value {
        let str = try args.unwrap1(location).unwrapString(location)
        let words = str
