...
extension Collection {
    ...
    func mustHaveAtLeast(_ n: Int, _ location: MyronLocation?) throws {
        if count < n { throw MyronError(.unexpectedArity(count, .atLeast(n)), at: location) }
    }

    func mustHaveAtMost(_ n: Int, _ location: MyronLocation?) throws {
        if count > n { throw MyronError(.unexpectedArity(count, .atMost(n)), at: location) }
    }

    func mustHaveExactly(_ n: Int, _ location: MyronLocation?) throws {
        if count != n { throw MyronError(.unexpectedArity(count, .exactly(n)), at: location) }
    }

    func headtail(_ location: MyronLocation?) throws -> (Element, Self.SubSequence) {
    ...
    func unwrap1(_ location: MyronLocation?) throws -> Element {
        guard count == 1 else {
    ...
    func unwrap2(_ location: MyronLocation?) throws -> (Element, Element) {
        guard count == 2 else {
    ...
    func unwrap3(_ location: MyronLocation?) throws -> (Element, Element, Element) {
        guard count == 3 else {
    ...
    func unwrapFirst(_ location: MyronLocation?) throws -> Element {
    ...
    func unwrapSecond(_ location: MyronLocation?) throws -> Element {
        guard count > 1 else {
    ...
    func unwrapThird(_ location: MyronLocation?) throws -> Element {
        guard count > 2 else {
